cat("#### Test byIndv4Times_GRsDiff with small example\n")
test_that("exampleData_byIndv4Times_GRsDiff", {
  ...
  t <- byIndv4Times_GRsDiff(data = longi.dat, response="PSA", times = "DAP", 
  ...
  t <- byIndv4Times_GRsDiff(data = subset(longi.dat, DAP %in% as.character((30:34))), 
                       response="PSA", times = "DAP", 
                       which.rates=c("AGR", "RGR"), 
                       ntimes2span = 3, avail.times.diffs = FALSE) 
  testthat::expect_equal(nrow(t), 100)
  testthat::expect_equal(ncol(t), 37)
  testthat::expect_true(all(unique(t$DAP) == c(30:34)))
